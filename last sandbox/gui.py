...
def display():
    e4["state"]="normal"
    listZ=stringToList(e4.get())
    e4["state"]="readonly"
    coords=GP.program(listZ)
    saveVar(coords)
    import display
    return
...
but6=Button(root,text="Display Graph",padx=70,pady=2,command=display,fg="green",bg="grey",state=DISABLED)
...
but6.grid(row=10,column=0,columnspan = 3)
