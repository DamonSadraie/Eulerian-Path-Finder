...
def shutgun():
    try:
        ...
        B=str(mC.getkmers(inputtext,inputk))
        e3["state"]="normal"
        e3.delete(0,"end")
        e3.insert(0,B)
        e3["state"]="readonly"
...
def reconstruct():
    ...
    e5.delete(0,END)
    str1=e4.get()
    list1=mC.stringToList(e4.get())
    list2=mC.program(list1)
    e5.insert(0,list2[1])
    e5["state"]="readonly"
    but5["state"]="normal"
    but6["state"]="normal"
...
but6=Button(root,text="Gif",padx=70,pady=2,command=mC.gifMaker,fg="blue",bg="grey",state=DISABLED)
